...
from core.dsl.ir import ScriptIR, FlowIR, ActionIR, InterruptIR
...
class DSLToYAMLAdapter:
    ...
    def convert(ir: ScriptIR) -> Script:
        ...
        # Flows
        flows = [
            DSLToYAMLAdapter._convert_flow(f)
            for f in ir.flows
        ]
        
        # Interrupts
        interrupts = [
            DSLToYAMLAdapter._convert_interrupt(i)
            for i in ir.interrupts
        ]
        ...
        return Script(
            ...
            main_flow="main" if any(f.name == "main" for f in flows) else (flows[0].name if flows else "main"),
        )
